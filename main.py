...
# para desativar o warning
st.set_option('deprecation.showPyplotGlobalUse', False)

st.write('# Análise de desempenho dos estudantes teste')
...
# st.dataframe(df, use_container_width=True) # mesma coisa
# df

# aba Geral
tab0, tab1, tab2, tab3, tab4 = st.tabs(
    tabs=['Geral', 'Etnia', 'Gênero', 'Desempenho notas', 'Desempenho materias'])
...
with tab1:
    ...
    total = 31.9
    f'''
    Analisando o grupo de etnias dos estudantes, o grupo C representa a maior proporção dos estudantes, representando {total}% do total
    '''
...
with tab2:
    ...
    plt.pie(x=df['gender'].value_counts(), labels=['Female', 'Male'], explode=[0, 0.1],
            autopct='%1.1f%%', shadow=True, colors=['#2c27fc', '#F97C06'])
...
with tab3:
    ...
    media = df_female['avg score'].mean().round(2)
    st.write(f'Analisando as estatísticas, o grupo feminino teve um desempenho maior que o masculino, com uma média de score de {media}')
